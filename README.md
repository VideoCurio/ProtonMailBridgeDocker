...
A docker version of the [Proton mail Bridge](https://proton.me/mail/bridge) command line interface. It creates a local
SMTP server, so other docker containers can send emails via your Proton email account.

> [!WARNING]  
> As of the time of this writing, you need a paid plan (Mail Plus, Proton Unlimited or Proton for Business)
> to be able to log in. It won't work for a free account.
...
> [!TIP]
> You can use the `-alpine` suffix to use the Alpine version of the docker image.  
> Be aware that both versions are roughly the same size (delta is less than 2MB).
>
> Example: `ghcr.io/videocurio/proton-mail-bridge:latest` and `ghcr.io/videocurio/proton-mail-bridge:latest-alpine`
...
1. Download the latest docker image from:

```bash
docker pull ghcr.io/videocurio/proton-mail-bridge:latest
```

2. **(Optional)** It is recommended to set up a custom docker network for all of your containers to use, for DNS /
...
docker run \
  ...
  --rm ghcr.io/videocurio/proton-mail-bridge:latest \
  cli
```

4. See the [Setup](#setup) section below to continue the setup.

5. Start the container in detached mode to run the bridge server:
   **You MUST provide a path volume storage** (`mkdir /path/to/your/volume/storage`).
...
docker run -d \
  ...
  -v /path/to/your/volume/storage:/root \
   ...
   --network network20 \
   --restart=unless-stopped ghcr.io/videocurio/proton-mail-bridge:latest
```

6. **(Optional)** You can check the bridge command line output with, you should see a bridge in ASCII art:
...
3. See the [Setup](#setup) section below to continue the setup.

4. Start the container in detached mode to run the bridge server:

```bash
docker compose up -d
```

5. **(Optional)** You can check the bridge command line output with, you should see a bridge in ASCII art:
...
**IF** you are using multiple domain names or email addresses, you **SHOULD** switch to split address mode (it will set
credentials for each address in the account).
...
Use the following information to connect via an SMTP client. The port numbers for the SMTP/IMAP connections are 12025
and 12143 (see command `docker ps`), **NOT** the one provided by the `info`command.

You **_MUST copy the username AND password_** from the info command (the password is random and different from your
Proton account):
...
See a list of [all Proton bridge commands available here](https://proton.me/support/bridge-cli-guide) or use the `help`
command.
...
```bash
docker container logs protonmail-bridge
```
...
1. **Image Configuration** - Image repository: `ghcr.io/videocurio/proton-mail-bridge` / Image tag: `latest` / Pull
   policy: `Always pull...`
2. **Container Configuration** - Entrypoint: `/app/entrypoint.sh` / Restart Policy: `Unless stopped`
3. **Network Configuration** - Add > Container Port: `25` / Host Port: `12025` (Or any other non-used port) / Protocol:
   `TCP`
4. **Storage Configuration** - Storage Add > Type: `Host path` / Mount Path: `/root` / Host path:
   `/mnt/path/to/your/protonmail-dataset`
...
1. **Container images** - Image repository: `ghcr.io/videocurio/proton-mail-bridge` / Image tag: `latest` / Image pull
   policy: `Always pull...`
2. **Container Entrypoint** - Command: `/app/entrypoint.sh`
3. **Container Environment Variables** - Add > Environment Variable Name: `PROTON_BRIDGE_SMTP_PORT` / Environment
   Variable Value: `1026`
...
* 2025/02/28: updated to Proton Mail Bridge v3.18.0, added environment variables CONTAINER_SMTP_PORT (default set to 25)
  and CONTAINER_IMAP_PORT (default set to 143), change this only if you have another MTA on the same docker network to
...
docker run -d --name=protonmail-bridge -v /path/to/your/volume/storage:/root -p 127.0.0.1:14025:25/tcp -p 127.0.0.1:14143:143/tcp --network network20 --restart=unless-stopped ghcr.io/videocurio/proton-mail-bridge:latest
docker container logs protonmail-bridge
...
There is a [testing branch](https://github.com/VideoCurio/ProtonMailBridgeDocker/tree/testing) available if you want to
submit a patch.

An [Alpine Linux](https://www.alpinelinux.org/) version for a small image base footprint is available in
the [Alpine directory](https://github.com/VideoCurio/ProtonMailBridgeDocker/tree/master/Alpine).
...
Made from [Debian 12 (bookworm) Go image](https://hub.docker.com/_/golang/) and
